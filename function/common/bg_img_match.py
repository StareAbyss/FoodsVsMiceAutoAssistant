    ...
    if source.shape[0] < template.shape[0] or source.shape[1] < template.shape[1]:
        ...
        SIGNAL.DIALOG.emit(
            ...
            text=f"问题: 图像识别模块 - 源图像大小: {source.shape}  < 目标图像大小: {template.shape}\n"
                 "可能原因1: 【未找到】游戏窗口\n"
                 "解决思路1: 正确的[360游戏大厅版本] [快速模式]，拖动窗口后，先[保存]再运行\n"
                 "可能原因2: 任务序列中【不含刷新】\n"
                 "解决思路2: 任务序列的第一项必须是刷新；若不是刷新需手动进入游戏而非服务器选择页面\n"
                 "可能原因3: 【最小化】游戏窗口\n"
                 "解决思路3: 确保游戏[非最小化运行], 被其他窗口遮住是没问题的\n"
                 "可能原因4: 【游戏窗口显示不全】部分在屏幕外\n"
                 "解决思路4: 游戏窗口必须完全拖回屏幕内\n"
                 "可能原因5: 【息屏】运行, 导致游戏图像不刷新\n"
                 "解决思路5: 保持亮屏, 或使用显卡欺骗器/虚拟显示屏\n"
                 "可能原因6: 【游戏加载过慢】, 刷新后等待足够时间后仍无法找到对应子窗口, 常见于空间服\n"
                 "解决思路6: 进阶设置 - 额外设置 - QQ空间额外设置, 应用刷新后额外休眠\n"
                 "可能原因7: 【游戏闪退】导致图像无法捕获\n"
                 "解决思路7: 无法解决, 请通过定时启动重启复位, 来解决问题\n"
                 "可能原因8: 【误报】中止任务线程时, 正好运行在这造成\n"
                 "解决思路8: 没卡住FAA可以不管这个报错\n"
        )
        return 0, None
...
def match_p_in_w(
    ...
    if test_print:
        CUS_LOGGER.debug(
            f"识别目标:{template_name}, 匹配度:{matching_degree:.5f}, 目标阈值:{match_tolerance:.5f}, 结果:成功")
