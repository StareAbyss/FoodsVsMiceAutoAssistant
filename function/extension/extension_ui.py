...
from extension_core import ExecuteThread


class CodeCodeEditWindow(QDialog):
...
class ImageSettingsWidget(QWidget):
    def __init__(self, index: int):
        ...
        # 偏移x
        self.offset_x_group = ParamGroupWidget("偏移x:", 0, 0, "", is_float=False, minimum=-2000, label_fixed_width=60,
                                               spin_fixed_width=60)
        click_input_layout.addWidget(self.offset_x_group)
        self.offset_x_group.setVisible(False)  # 初始状态下隐藏

        self.offset_y_group = ParamGroupWidget("偏移y:", 0, 0, "", is_float=False, minimum=-2000, label_fixed_width=60,
                                               spin_fixed_width=60)
    ...
    def show_code_code_edit(self):
        ...
        code_edit = CodeCodeEditWindow(self)
...
class ExtensionMainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def init_ui(self):
        ...
        # 循环执行次数
        self.loop_group = ParamGroupWidget("循环执行次数", 1, 0, "次", is_float=False, maximum=9999,
                                           label_fixed_width=100, spin_fixed_width=60)
...
def faa_extension_start_main():
    app = QApplication(sys.argv)
    window = ExtensionMainWindow()
    window.show()
    sys.exit(app.exec())
