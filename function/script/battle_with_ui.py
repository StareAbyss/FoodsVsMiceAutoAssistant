...
from function.script.load_opt import MyMainWindow1
from function.tools.get_channel_name import get_channel_name
...
class Todo(QThread):
    ...
    def n_battle(
        ...
        if is_group:
            while True:
                ...
                if invite(self.faa_2, self.faa_1):
                    ...
                    self.faa_1.action_goto_exit(3)
                    self.faa_2.action_goto_exit(3)
        # 单人前往副本
        else:
        ...
        for i in range(max_times):
            ...
            # 创建战斗进程 -> 开始进程
            self.thread_1p = ThreadWithException(target=battle_a_round,
                                                 name="1P Thread",
                                                 kwargs={"faa": self.faa_1,
                                                         "delay_start": False,
                                                         "battle_mode": 0,
                                                         "task_card": task_card,
            ...
            if is_group:
                self.thread_2p = ThreadWithException(target=battle_a_round, name="2P Thread",
                                                     kwargs={"faa": self.faa_2,
                                                             "delay_start": True,
                                                             "battle_mode": 0,
                                                             "task_card": task_card,
                                                             "list_ban_card": list_ban_card,
            ...
            if i + 1 == max_times:
                for j in dict_exit["last_time"]:
                    self.faa_1.action_goto_exit(exit_mode=j)
                    if is_group:
                        self.faa_2.action_goto_exit(exit_mode=j)
            else:
    ...
    def double_task(
        ...
        self.faa_1.action_completed_task(task_type)
        self.faa_2.action_completed_task(task_type)
        ...
        for i in range(max_times):
            ...
            self.thread_1p = ThreadWithException(target=battle_a_round,
                                                 ...
                                                 kwargs={"faa": self.faa_1,
                                                         ...
                                                         "list_ban_card": [],
                                                         "deck": deck})
            self.thread_2p = ThreadWithException(target=battle_a_round,
                                                 name="2P Thread",
                                                 kwargs={"faa": self.faa_2,
                                                         "delay_start": True,
                                                         "battle_mode": 0,
            ...
            if i + 1 == max_times:
                # 最后一把
                self.faa_1.action_goto_exit(exit_mode=1)
                self.faa_2.action_goto_exit(exit_mode=2)
            else:
                # 其他次数
                self.faa_1.action_goto_exit(exit_mode=1)
                self.faa_2.action_goto_exit(exit_mode=0)
    ...
    def alone_magic_tower(
        ...
        for count_times in range(max_times):
            ...
            self.thread_1p = ThreadWithException(target=battle_a_round,
                                                 name="1P Thread",
                                                 kwargs={"faa": self.faa_1, "deck": deck,
                                                         "delay_start": False,
                                                         "battle_mode": 0,
                                                         "task_card": "None",
                                                         "list_ban_card": []
            ...
            if count_times + 1 == max_times:
                self.faa_1.action_goto_exit(exit_mode=2)
            else:
                self.faa_1.action_goto_exit(exit_mode=0)
    ...
    def customize_battle(self, text_: str):
        ...
        with open(self.faa_1.path_config + "//opt_customize_todo.json", "r", encoding="UTF-8") as file:
