...
from function.common.bg_screenshot_and_compare import loop_find_p_in_w, find_p_in_w, loop_find_ps_in_w
...
def round_of_game(
    ...
    # 循环查找火苗图标 找到战斗开始
    my_path = paths["picture"]["common"] + "\\battle_fire_element.png"
    if not loop_find_p_in_w(handle=handle,
                            target_path=my_path,
                            click_zoom=zoom,
                            target_interval=1,
    ...
    # 理想情况下,此处应该是[战利品]界面
    f1 = find_p_in_w(handle=handle, target_tolerance=0.999, target_path=paths["picture"]["common"] + "\\battle_end_1_loot.png")
    f2 = find_p_in_w(handle=handle, target_tolerance=0.999, target_path=paths["picture"]["common"] + "\\battle_end_2_loot.png")
    if f1 or f2:
    ...
    # 循环查找[翻宝箱], 确认是否可以安全翻牌
    find = loop_find_p_in_w(handle=handle, click_zoom=zoom, click=False, target_failed_check=15,
    ...
    find = loop_find_ps_in_w(handle=handle,
                             target_opts=[{"target_path": paths["picture"]["common"] + "\\battle_before_ready_check_start.png",
                                           "tolerance": 0.99},
