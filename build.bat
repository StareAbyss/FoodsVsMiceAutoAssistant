...
IF "%~1"=="" (
    echo.
    echo ❌ 错误：请通过参数传入UPX工具的安装路径。
    echo    用法示例: .\build.bat "D:\Program Files\upx-4.2.4-win64"
    pause
    goto :eof
)
set "UPX_PATH=%~1"

REM 检查 upx.exe 是否存在于传入的路径中
IF NOT EXIST "%UPX_PATH%\upx.exe" (
    echo.
    echo ❌ 错误：在指定的UPX路径中未找到 upx.exe 文件。
...
echo ----------------------------------------------------
REM ----------------------------------------------------
REM --- 3. 打包后执行自动化脚本 (优化中文输出) ---
REM ----------------------------------------------------

REM 检查打包后脚本是否存在
IF NOT EXIST "%POST_BUILD_SCRIPT%" (
    echo.
    echo 警告：打包后脚本 "%POST_BUILD_SCRIPT%" 不存在，跳过执行资源文件处理。
...
REM 使用当前环境中的 python 命令执行脚本
python "%POST_BUILD_SCRIPT%"

REM 检查 python 脚本的退出代码
IF %ERRORLEVEL% NEQ 0 (
    echo.
    echo 警告：资源文件脚本执行失败，退出代码 %ERRORLEVEL%，请检查脚本输出。
) ELSE (
    echo.
    echo 资源文件脚本执行成功。
)
