...
- [ ] 隔离大二进制和生成文件。
  - 根目录存在约 65MB 的 `自定义识图脚本插件...exe`。
  - `function/qrc/test_rc.py` 约 7.4MB，属于生成文件。
  - 建议生成文件由构建脚本产生，大二进制放 Release 或单独下载。
...
- [ ] 梳理测试目录。
  - `test/` 下混有实验脚本、图片资源、正式测试。
  - 建议拆成 `tests/`、`sandbox/`、`test_assets/` 或类似结构。
