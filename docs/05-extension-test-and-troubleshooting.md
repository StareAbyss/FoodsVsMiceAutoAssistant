...
这个目录同时包含正式回归测试、可复现诊断脚本和人工环境测试，尚未全部接入统一测试框架。

测试生成的截图、CSV、日志和其他本地研究产物不提交到 Git，统一写入对应测试旁边的
`output/` 目录，并由 `.gitignore` 按文件类型排除。详细约定见 `test/README.md`。
...
- 新增测试脚本：
  ...
  - 包含 Python 模块的测试子目录必须保留 `__init__.py`
  - 生成文件通过 `test.output_paths.get_test_output_dir()` 保存
