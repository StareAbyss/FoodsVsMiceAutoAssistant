...
from function_common.background_screenshot_and_compare_picture import loop_find_p_in_p_ml_click, find_p_in_p
...
def battle_a_round(
    ...
    img = vconcat(img)
    # 保存图片
    imwrite("{}\\{}_{}.png".format(path_p_logs, stage_name, strftime('%Y-%m-%d_%Hh%Mm', localtime())), img)
...
class AutoCompleteToDoList:
    ...
    def __init__(self):
        # 打包后exe所在目录
        exe_path = str(os.path.dirname(os.path.realpath(sys.executable)))
        with open(exe_path + "//todo.json", "r", encoding="UTF-8") as file:
